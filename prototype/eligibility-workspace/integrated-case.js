(function () {
  ...
  function renderFinancial(ctx) {
    ...
    const utilityFields = d.expenses.utilities.map((utility, index) => fields(d, [
      { path: `expenses.utilities.${index}.type`, label: `Utility ${index + 1} type`, type: "select", options: ["Electricity", "Heating fuel", "Gas", "Water/sewer", "Telephone", "Trash", "Other"] },
      { path: `expenses.utilities.${index}.arrangement`, label: `Utility ${index + 1} arrangement`, type: "select", options: ["Paid separately", "Included in rent", "Shared", "Reimbursed", "Other"] },
      { path: `expenses.utilities.${index}.amount`, label: `Utility ${index + 1} amount`, type: "currency" },
      { path: `expenses.utilities.${index}.frequency`, label: `Utility ${index + 1} frequency`, type: "select", options: ["Weekly", "Monthly", "Quarterly", "Annual"] },
      { path: `expenses.utilities.${index}.shared`, label: `Utility ${index + 1} cost shared`, type: "select", options: YES_NO_UNKNOWN },
    ])).join("");
    const dependentCareFields = d.expenses.dependentCare.map((expense, index) => fields(d, [
      { path: `expenses.dependentCare.${index}.person`, label: `Care expense ${index + 1} person`, type: "select", options: d.people.map((person) => person.name) },
      { path: `expenses.dependentCare.${index}.reason`, label: `Care expense ${index + 1} reason`, type: "select", options: ["Employment", "Training or education", "Medical need", "Other"] },
      { path: `expenses.dependentCare.${index}.provider`, label: `Care expense ${index + 1} provider`, type: "text" },
      { path: `expenses.dependentCare.${index}.amount`, label: `Care expense ${index + 1} amount`, type: "currency" },
      { path: `expenses.dependentCare.${index}.frequency`, label: `Care expense ${index + 1} frequency`, type: "select", options: ["Weekly", "Every two weeks", "Twice monthly", "Monthly"] },
      { path: `expenses.dependentCare.${index}.subsidized`, label: `Care expense ${index + 1} subsidized`, type: "select", options: YES_NO_UNKNOWN },
    ])).join("");
    const commonExpense = `${fields(d, [
      ...
      { path: "expenses.utilitiesStatus", label: "Utility expenses", type: "select", options: ["None", "One or more", "Unknown"] },
      { path: "expenses.dependentCareStatus", label: "Dependent-care expenses", type: "select", options: ["None", "One or more", "Unknown"] },
    ])}${utilityFields}${dependentCareFields}<div class="bc-mini-repeaters"><article><strong>Utilities</strong><span>${d.expenses.utilities.length} record · ${d.expenses.utilities[0]?.type || "None"}</span><small>${d.expenses.utilities[0]?.arrangement || "Not answered"} · $${d.expenses.utilities[0]?.amount || "0"}</small></article><article><strong>Dependent care</strong><span>${d.expenses.dependentCareStatus}</span><small>${d.expenses.dependentCare[0]?.provider || "No provider"}</small></article>${ctx.scenario.programs.includes("Medicaid") ? `<article><strong>Medical expenses</strong><span>${escapeHTML(d.expenses.medicalStatus)}</span><small>Shown for pathway training; no deduction is calculated</small></article>` : ""}</div>`;
