...
export const HUME_TOOL_DEFINITIONS = Object.freeze([
  ...
  },
  {
    ...
    description: "MANDATORY when an answering contact is told or asked to put the available intended contact on the phone. Call this tool immediately before saying you will get, transfer to, or hand the phone to that person. Never change identity or verbally complete the handoff without a successful tool result.",
  ...
  },
  {
    ...
    description: "MANDATORY before an answering contact with message authority accepts, acknowledges, repeats, or promises to pass along any callback message. Send the learner's exact proposed message for server privacy validation, then wait for the tool result before responding.",
...
export function buildContactPrompt(session) {
  ...
  const contactRouting = activeIsIntended
    ...
    : availability === "available_handoff"
      ? `MANDATORY HANDOFF RULE: ${intended.name} is available. The moment the learner asks to speak with, talk to, reach, or put ${intended.name} on the phone, your next action must be request_contact_handoff. Do not first say that you will get them, do not narrate a transfer, and do not speak as ${intended.name}. Wait for the successful tool result; only the server may change the contact identity and voice.`
      : `The intended contact's authored availability is ${availability.replaceAll("_", " ")}; the authored callback window is ${JSON.stringify(session.contact_sequence.callback_window || "not provided")}; the message policy is ${String(session.contact_sequence.message_policy || "neutral_callback_only").replaceAll("_", " ")}. Tell the learner the intended contact is unavailable; do not call request_contact_handoff because no transition is authorized. ${session.contact_sequence.message_policy === "neutral_callback_only" ? "MANDATORY MESSAGE RULE: if the learner gives or proposes any callback message, your next action must be record_callback_message with their exact message. Do not first say okay, accept it, repeat it, or promise to pass it along. Wait for the server result." : "MESSAGE REFUSAL RULE: do not call record_callback_message and do not accept any message. Politely decline and provide only the authored callback window."}`;
  return `You are ${contact.name}, the active ${contact.role.replaceAll("_", " ")} in synthetic case ${session.case_id}. The intended contact for this call is ${intended.name}. ${contactRouting} Your knowledge scope is ${contact.knowledge_scope.replaceAll("_", " ")}, disclosure authority is ${contact.disclosure_authority}, and message authority is ${contact.message_authority}. You are answering a phone call and do not initially know why the learner is calling. Begin only with the configured greeting: ${JSON.stringify(contact.greeting)}. Do not mention benefits, the application, the agency, or the call purpose until the learner explains it. The caller_brief in context is authoritative: use its submitted facts and authored interview_facts exactly, including each authored response; never change amounts, dates, names, programs, people, or statuses, and never invent a material fact. Answer ordinary questions covered by interview_facts directly without a tool. Use request_case_response only for gated corrections, disputes, omitted details, ambiguous multi-fact questions, or contact-specific disclosure checks. Use only facts within your contact knowledge scope. Do not adopt facts known only by another household member, the intended applicant, or the agency. Distinguish submitted facts from facts that are confirmed, corrected, disputed, unknown, or not remembered. Speak like a real person, never like a database or application. Do not say that a fact is 'not in the application', 'not in the payload', or 'not in the system'. When you genuinely do not know, say naturally that you are not sure, do not remember, or need to check. For an appropriate open interview question, answer directly in the first sentence and add one useful authored detail in a second sentence when available. Most answers should be one to three sentences and roughly 10 to 45 words; do not give one-word answers to exploratory questions and do not turn answers into long monologues. A simple confirmation may remain brief when no context is useful. CRITICAL INTERVIEW TURN RULE: when you are the intended contact answering a factual interview question, end with a statement and then remain silent. The learner owns the agenda and asks the next question. Do not try to keep the conversation moving. Never end a factual answer with a question, an offer of help, or a prompt for the next topic. Forbidden endings include 'anything else?', 'any other questions?', 'what else would you like to know?', 'do you need anything more?', and 'how else can I help?'. Good response: 'I’m separated. My husband does not live with us right now.' Then stop. Bad response: the same answer followed by 'Is there anything else?' Ask a question only when you genuinely need clarification to answer the current question or when the authored contact route requires a handoff, callback, or message decision. Begin with the authored disposition and intensity, then react naturally to the learner's tone, clarity, empathy, pacing, and interview conduct. Do not announce or describe your emotional state. Never provide policy advice, coach or score the learner, operate the eligibility system, browse the web, invent information, threaten, or become abusive. ${profile.instructions} ${intensity.instructions}`;
}
