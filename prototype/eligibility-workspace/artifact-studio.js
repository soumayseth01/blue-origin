...
function renderNotebookSettings() {
  ...
  const heygenLabel = status.heygen ? "Connected" : status.heygen_configured ? "Needs attention" : "Not configured";
  const integrations = [["Open Notebook", status.notebook, "Source catalog, lineage, and release records", status.notebook ? "Configured" : "Not configured"], ["OpenAI", status.openai, "Source ranking, cited answers, brief editing, and narration", status.openai ? "Configured" : "Not configured"], ["HeyGen", status.heygen, status.heygen_error || "Per-slide avatar rendering for presentation videos", heygenLabel], ["Artifact worker", status.worker, "DOCX, PDF, PPTX, captions, and media assembly", status.worker ? "Configured" : "Not configured"]];
  return `<div class="product-page"><section class="list-heading"><div><span class="page-kicker">Manage</span><h2>Settings</h2><p>Server-side integration health. Secret values are never returned to the browser.</p></div><button class="button button-secondary" data-action="refresh-integration-status">Refresh status</button></section><section class="settings-list">${integrations.map(([name, configured, text, label]) => `<article><div><strong>${name}</strong><p>${escapeHTML(text)}</p></div><span class="status-chip ${configured ? "" : "neutral"}">${label}</span></article>`).join("")}</section><section class="credential-boundary"><span>${materialIcon("lock")}</span><div><strong>Credentials stay server-side.</strong><p>OPENAI_API_KEY, HEYGEN_API_KEY, Blob, Notebook, and queue credentials are deployment secrets and are never saved in projects or release manifests.</p></div></section></div>`;
}
